...
const Select_Menu = ({ item, values, setValues }) => {
  ...
  return (
    <div className='select-wrapper'>
      ...
      <select name={item.value} id="" required className='select text-regular' onInvalid={(e) => e.target.setCustomValidity(`${item.tooltip}`)} onInput={(e) => e.target.setCustomValidity('')} onChange={handleChange}>
        {
