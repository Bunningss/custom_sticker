    ...
    <div className='edit-order default'>
        ...
        <div className="row">
            <form className="order-status" onSubmit={handleSubmit}>
                ...
                <div className="text-medium">Current Status: <span>{order.status}</span></div>
                <select name="OrderStatus" className='input' required onChange={(e) => setStatus(e.target.value)} onInvalid={(e) => e.target.setCustomValidity("Please Select Order Status")} onInput={(e) => e.target.setCustomValidity("")}>
